...
def draw_board(solution):
    img = Image.new( 'RGB', (9*40+1,9*40+1), "white")
    # Create ImageDraw
    idraw = ImageDraw.Draw(img)
    # Itterate over all nodes
    font = ImageFont.truetype("FreeSerif.otf", 24)

    for row in range(9):
        for col in range(9):
            # Get correct color            
            c = (255,255,255)
            # Draw rectangle 40x40 with color
            idraw.rectangle([(row*40,col*40),(row*40+40,col*40+40)], fill=c, outline=(0,0,0))
            # If path, draw a dot
                
            idraw.text((row*40+14,col*40+8), solution['%d-%d' % (row, col)][0], font=font,fill=(0,0,0,0))
        
    for row in range(9):
        for col in range(9):
            if row == 3 or row == 6:
                idraw.line(((0,row*40),(col*40+40,row*40)), fill=(0,0,0), width=2)

            if col == 3 or col == 6:
                idraw.line(((col*40,0),(col*40,row*40+40)), fill=(0,0,0), width=2)

    img.save("hard.png","PNG")

if __name__ == "__main__":
    csp = create_sudoku_csp("hard.txt")
    solution = csp.backtracking_search()
    #print solution
    print '\n',"bCounter: " ,csp.back_track_counter, "Fail: ", csp.failiure_counter
    if solution:
